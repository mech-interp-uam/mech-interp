...
from datasets import load_dataset, Datasettw
...
# ====================================================================================
# Carga del modelo SAE y preparación del dataset
# ====================================================================================
...
# ====================================================================================
# Construir un mapa de doc_id a listas de (tok_pos, token_id)
# ====================================================================================
doc_tokens_map = {}

print("Pre-procesando dataset para mapear doc_id a tokens (esto puede tardar y usar RAM)...") #cambiar para no cargar todo en RAM

temp_ds_iter = raw_ds.with_format(None)
for item in tqdm(temp_ds_iter, desc="Building doc-token map"):
    doc_id = item['doc_id']
    tok_pos = item['tok_pos']
    token_id = item['input_ids'] # Aqui es un tensor,  se convierte a item() to item() si aun no se ha hecho con with_format("None")
...
# ====================================================================================
# Ordena los tokens dentro de cada documento y extrae solo los token_ids
# ====================================================================================
for doc_id in doc_tokens_map:
    doc_tokens_map[doc_id].sort(key=lambda x: x[0]) # Sort by tok_pos
    doc_tokens_map[doc_id] = [x[1] for x in doc_tokens_map[doc_id]] # Mantiene solo los token_id
...
# ====================================================================================
# Funciones para explicar neuronas usando OpenAI
# ====================================================================================
...
# ====================================================================================
# Validación del encoder y análisis de activaciones
# ====================================================================================  
if __name__ == "__main__":
    ...
    counts_filename = "neuron_activation_counts.npy"
    try:
        np.save(counts_filename, counts)
        print(f"✅ Contador de activaciones guardado en '{counts_filename}'")
    except Exception as e:
        print(f"❌ Error al guardar el contador de activaciones: {e}")
    ...
    plt.hist(counts, bins=range(0, int(counts.max()) + 2, 1), edgecolor='black', alpha=0.7, log=True)
    plt.title('Distribución de Activaciones de Neuronas SAE')
    plt.xlabel('Número de Activaciones (counts)')
    plt.ylabel('Número de Neuronas')
    plt.grid(axis='y', alpha=0.75)
    plt.text(0.95, 0.95, f'Total Neuronas: {D_SAE:,}\nNeuronas Vivas: {live:,}\nMedia Activaciones: {counts.mean():.1f}',
             transform=plt.gca().transAxes, fontsize=10, verticalalignment='top', horizontalalignment='right',
             bbox=dict(boxstyle='round,pad=0.5', fc='wheat', alpha=0.5))
    histogram_filename = "neuron_activation_histogram.png"
    plt.savefig(histogram_filename)
    print(f"Histograma guardado como '{histogram_filename}'")
    results = []
    for nid in tqdm(active_ids, desc="Explicando neuronas"):
        ...
        print(explanation_data["ai_explanation"]) 
